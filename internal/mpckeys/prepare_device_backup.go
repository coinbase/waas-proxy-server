...
// PrepareDeviceBackup proxies the PrepareDeviceBackup method.
func (m *mpcKeysProxy) PrepareDeviceBackup(
	ctx context.Context,
	req *v1.PrepareDeviceBackupRequest,
) (*longrunningpb.Operation, error) {
	m.log.Info("PrepareDeviceBackup")

	wrappedOp, err := m.mpcKeysClient.PrepareDeviceBackup(ctx, req)
	if err != nil {
		return nil, err
	}
